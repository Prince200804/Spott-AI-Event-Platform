...
export const getEventDashboard = query({
  ...
  handler: async (ctx, args) => {
    ...
    const checkedInCount = confirmedRegs.filter((r) => r.checkedIn).length;
    const pendingCount = totalRegistrations - checkedInCount;

    // Calculate revenue from actual payments
    const paidRegs = confirmedRegs.filter((r) => r.paymentStatus === "paid");
    const paidCount = paidRegs.length;
    const unpaidCount = confirmedRegs.filter((r) => r.paymentStatus === "pending").length;
    let totalRevenue = 0;
    if (event.ticketType === "paid") {
      totalRevenue = paidRegs.reduce((sum, r) => sum + (r.amountPaid || event.ticketPrice || 0), 0);
    }
